...
void MeshCollider::Initialize() {
	...
	for(const auto& vertexData : pModel_->GetVertexDatas()) {
		vertices_.push_back(Mat4::Transform(Vec3f::Convert4To3(vertexData.position), pWorldTarnsform_->matTransform));
	}
...
bool MeshCollider::IsCollision(const std::vector<Vec3f>& vertices, const WorldTransform& worldTransform) {
	...
	for(const auto& vertex : vertices) {
		worldVertices.push_back(Mat4::Transform(vertex, worldTransform.matTransform));
	}
